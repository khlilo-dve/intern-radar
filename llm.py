...
2. 字段与类型必须严格遵循：
   ...
   - Tech_Vision: integer 0-100 — 技术视野要求。评分标准：80+ 要求系统架构/底层原理级理解；60-79 需要技术方案选型能力；40-59 仅需了解技术概念；<40 纯业务岗无技术要求
   - Product_Dominance: integer 0-100 — 产品业务主导权。评分标准：80+ 独立负责产品线/业务决策；60-79 参与产品规划有话语权；40-59 执行层但有建议权；<40 纯执行无产品参与
   - Exec_Ratio: integer 0-100 — 日常执行杂活比例（越低越好）。评分标准：<30 以策略/架构/决策为主；30-49 创作与执行各半；50-69 执行为主偶有规划；70+ 纯重复性劳动
   - AI_Leverage: integer 0-100 — AI/自动化杠杆空间。评分标准：80+ 核心工作流可被 AI 显著加速；60-79 部分环节可自动化；40-59 少量辅助场景；<40 纯人工密集型
   - Growth_Ceiling: integer 0-100 — 成长天花板。评分标准：80+ 清晰的技术+业务双晋升路径；60-79 有成长空间但路径模糊；40-59 天花板较低；<40 死胡同岗位
   - Match_Score: integer 1-100 — 综合匹配度。加权公式：Tech_Vision×0.25 + Product_Dominance×0.25 + (100-Exec_Ratio)×0.20 + AI_Leverage×0.15 + Growth_Ceiling×0.15。评分标尺：>85 必须包含高技术杠杆或产品主导权；<50 为需要避开的纯重复性劳动。若 Red_Flags 非空，分数不得超过 60。
