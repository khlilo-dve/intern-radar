...
│  多 URL 并行处理（ThreadPoolExecutor, max_workers=4）│
│  截图模式：Vision OCR 提取文字 → 文本模型分析评分    │
└──────────────────────────────────────────────────────┘
...
- **多 URL 并行解析**：一条消息中多个链接并行处理（ThreadPoolExecutor，最多 4 并发）
- **截图两步解析**：Vision 模型只做 OCR 提取文字，文本模型负责分析评分，速度更快
...
crawler:
  ...
  sources:
    - name: nowcoder         # 牛客实习广场（推荐，直爬无依赖）
      enabled: true
      platform: nowcoder
      feeds:
        - keyword: "AI产品"  # 关键词过滤
          city: "北京"       # 城市限制
    - name: boss             # Boss 直聘（需 playwright）
      enabled: false
      platform: boss
      feeds:
        - keyword: "AI产品实习"
...
- 一条消息多个 URL → **并行解析入库**（最多 4 个并发）
- 富文本消息（含链接+图片混合）→ 全部识别处理
- 任意招聘页截图 → **Vision OCR 提取文字 → 文本模型分析评分** + 自动补搜索链接
- 纯聊天 → 回"没 URL，跳过"
...
├── handlers.py              # 消息分发 + 卡片回复 + 去重 + 高分推送 + 多URL并行
├── llm.py                   # OpenAI 兼容调用 + 多维评分 prompt + 搜索链接构造 + 截图两步解析
...
### 多 URL 并行处理
- 单 URL：走原路径，不额外开线程
- 多 URL：`ThreadPoolExecutor` 并行抓取 + LLM 分析，最多 4 个并发
- 总耗时 = 最慢那条，而非所有链接之和

### 截图两步解析
- 改前：Vision 模型一步到位（OCR + 评分），~94s
- 改后：Vision 只做 OCR 提取文字，文本模型负责分析评分，~72s
- 提速约 24%，因为 Vision 的 prompt 从几千字砍到一句话
...
| 牛客爬虫抓不到数据 | 检查网络是否能访问 nowcoder.com，爬虫带 3 次重试 |
| Boss 爬虫不工作 | 需要 `pip install playwright && playwright install chromium` |
...
### 2.0.2
- 多 URL 并行处理（ThreadPoolExecutor，最多 4 并发）
- 截图两步解析：Vision 只做 OCR，文本模型负责分析评分，提速约 24%
...
### 2.0.0
- 新增 Boss 直聘爬虫 + 修复 trafilatura 调用
